...
def init(config_name='profile.yml'):
    ...
    if not os.path.exists(chaorenpath.CONFIG_PATH):
        try:
            os.makedirs(chaorenpath.CONFIG_PATH)
        except OSError:
            _logger.error("Could not create config dir: '%s'",
                          chaorenpath.CONFIG_PATH, exc_info=True)
            raise
    ...
    config_file = chaorenpath.config(config_name)
    global _config
