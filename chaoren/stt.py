...
class SnowboySTT(AbstractSTTEngine):
    ...
    Snowboy STT 离线识别引擎（只适用于离线唤醒）
        ...
        snowboy:
            model: '/home/pi/.chaoren/snowboy/dingdangdingdang.pmdl'  # 唤醒词模型
            sensitivity: "0.5"  # 敏感度
        ...
    """
    ...
    def __init__(self, sensitivity, model, hotword, **kwargs):
        ...
        self.model = model
        self.resource_file = os.path.join(chaorenpath.LIB_PATH,
                                          'snowboy/common.res')
        try:
    ...
    def get_config(cls):
        ...
        if 'model' not in profile:
            profile['model'] = os.path.join(
                chaorenpath.LIB_PATH, 'snowboy/chaoren.pmdl')
